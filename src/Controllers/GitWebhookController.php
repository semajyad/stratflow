...
// Step 2 — extract installation_id for multi-tenant routing
        ...
        if ($githubEvent === 'push') {
            ...
            $repoRow = IntegrationRepo::findByIntegrationAndGithubId($this->db, (int) $integration['id'], $pushData['repo_github_id']);
            if ($repoRow === null) {
                http_response_code(200);
            ...
            $affected = 0;
            $aiQueue  = [];
// commits with no explicit SF- tag go to AI matching

            foreach ($pushData['commits'] as $commit) {
                $subject = strtok($commit['message'], "\n");
// first line only
                if (preg_match('/\b(SF|StratFlow)[-_\s]?(\d+)\b/i', $commit['message'])) {
                    $linked = $service->linkFromPrBody($commit['message'], $commit['url'], 'github', $subject, 'merged', $commit['author'] ?? null);
                    $affected += $linked;
                } else {
                    $aiQueue[] = $commit;
                }
            ...
            \StratFlow\Services\Logger::warn(sprintf('[GitWebhook] GitHub push %s branch=%s org_id=%d commits=%d explicit_links=%d ai_queue=%d', $pushData['repo_full_name'], $pushData['branch'], $orgId, count($pushData['commits']), $affected, count($aiQueue)));
            http_response_code(200);
...
// AI matching for commits with no explicit reference — fire after response
            if (!empty($aiQueue)) {
                ...
                foreach ($aiQueue as $commit) {
                    $subject = strtok($commit['message'], "\n");
                    $matcher->matchAndLink($subject, $commit['message'], $pushData['branch'], $commit['url'], $orgId);
                }
            }
            return;
        }
...
// Step 5 — only pull_request events produce links
        ...
        $repoRow = IntegrationRepo::findByIntegrationAndGithubId($this->db, (int) $integration['id'], $event['repo_github_id']);
        if ($repoRow === null) {
            \StratFlow\Services\Logger::warn(sprintf('[GitWebhook] GitHub: repo_github_id=%d not on allowlist for installation_id=%d', $event['repo_github_id'], $installationId));
            http_response_code(200);
            echo json_encode(['ok' => true, 'links_affected' => 0, 'reason' => 'repo_not_linked']);
            return;
        }
        ...
        \StratFlow\Services\Logger::warn(sprintf('[GitWebhook] GitHub PR %s action=%s org_id=%d links_affected=%d', $event['pr_url'], $event['action'], $orgId, $affected));
        http_response_code(200);
        echo json_encode(['ok' => true, 'links_affected' => $affected]);
...
// AI PR matching: fires on opened/reopened/synchronize with no explicit tag found
        if ($affected === 0 && in_array($event['action'], ['opened', 'reopened', 'synchronize'], true)) {
            $matcher = new GitPrMatcherService($this->db, $gemini);
            $matcher->matchAndLink($event['title'], $event['body'], $event['branch'] ?? '', $event['pr_url'], $orgId);
        }
    ...
    private function handleInstallationReposEvent(array $payload, int $installationId): void
    {
        ...
        \StratFlow\Services\Logger::warn(sprintf('[GitWebhook] GitHub: installation_repositories installation_id=%d added=%d removed=%d', $installationId, count($event['added']), count($event['removed'])));
    }
    ...
    {
        $stmt = $this->db->query("SELECT config_json FROM integrations
    ...
    private function dispatchEvent(GitLinkService $service, array $event, string $provider): int
    {
        if (in_array($event['action'], ['opened', 'reopened', 'synchronize'], true)) {
            return $service->linkFromPrBody($event['body'], $event['pr_url'], $provider, $event['title'], 'open', $event['author'] ?? null);
        }
