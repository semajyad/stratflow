...
// Force strictly valid UTF-8 sequences (scrubs malformed AI byte blobs)
...
// Attempt 1: plain decode (fast path - works when AI returns clean JSON)
        ...
        $decoded   = json_decode($sanitized, true, 512, JSON_INVALID_UTF8_IGNORE);
        if (json_last_error() === JSON_ERROR_NONE) {
            return $decoded;
        }
    ...
    private function sanitizeJsonControlChars(string $text): string
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($ord < 0x20) {
                ...
                // Outside a string, only JSON whitespace is allowed: \t, \n, \r
                    if ($ord === 0x09 || $ord === 0x0A || $ord === 0x0D) {
                        $out .= $c;
                    }
                    // Otherwise (e.g. \x00, \x0B, \x1B), completely strip it to prevent parse failure
                }
                continue;
            }
